  ...
  return (
    <div style={{ minHeight: "100vh", background: DARK, fontFamily: "'Segoe UI', system-ui, sans-serif", color: TEXT, maxWidth: 430, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "16px 20px 14px", background: `linear-gradient(180deg, ${CARD}, transparent)`, borderBottom: `1px solid ${ORANGE}22`, position: "sticky", top: 0, zIndex: 100, backdropFilter: "blur(12px)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 36, height: 36, background: `linear-gradient(135deg, ${ORANGE}, ${ORANGE_LIGHT})`, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>🛒</div>
            <div>
              <div style={{ fontSize: 15, fontWeight: 800, color: TEXT }}>Shopee Ads Pro</div>
              <div style={{ fontSize: 10, color: MUTED }}>Gerenciador de Campanhas</div>
            </div>
          </div>
          {salvou ? (
            <div style={{ background: `${GREEN}22`, border: `1px solid ${GREEN}44`, borderRadius: 999, padding: "4px 10px", fontSize: 10, color: GREEN }}>✅ Salvo!</div>
          ) : (
            <div style={{ background: `${GREEN}22`, border: `1px solid ${GREEN}44`, borderRadius: 999, padding: "4px 10px", fontSize: 10, color: GREEN, display: "flex", alignItems: "center", gap: 5 }}>
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: GREEN, animation: "pulse 2s infinite" }} /> AO VIVO
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, padding: "16px 16px 90px", overflowY: "auto" }}>
        ...
        {aba === "dashboard" && (
          <div>
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: MUTED, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Resumo Atual</div>
              <div style={{ background: `linear-gradient(135deg, ${ORANGE}22, ${CARD})`, border: `1px solid ${ORANGE}33`, borderRadius: 20, padding: 20, display: "flex", gap: 16, alignItems: "center" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, color: MUTED }}>Receita Total</div>
                  <div style={{ fontSize: 30, fontWeight: 900, color: ORANGE, fontFamily: "monospace" }}>R${metricas.receita.toFixed(2).replace(".", ",")}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 11, color: MUTED }}>Gasto</div>
                  <div style={{ fontSize: 20, fontWeight: 800, color: TEXT, fontFamily: "monospace" }}>R${metricas.gasto.toFixed(2).replace(".", ",")}</div>
                  <div style={{ fontSize: 11, color: metricas.roas >= 3 ? GREEN : RED, marginTop: 4 }}>ROAS {metricas.roas}x {metricas.roas >= 3 ? "✅" : "⚠️"}</div>
                </div>
              </div>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              <MetricCard label="Impressões" value={metricas.impressoes} format="integer" color={ORANGE} icon="👁️" delay={0} />
              <MetricCard label="Cliques" value={metricas.cliques} format="integer" color={ORANGE_LIGHT} icon="👆" delay={100} />
              <MetricCard label="CTR" value={metricas.ctr} format="percent" color={YELLOW} icon="🎯" delay={200} />
              <MetricCard label="CPC" value={metricas.cpc} format="decimal" color={GREEN} icon="💸" delay={300} />
              <MetricCard label="Pedidos" value={metricas.pedidos} format="integer" color={GREEN} icon="📦" delay={400} />
              <MetricCard label="Conversão" value={metricas.conversao} format="percent" color="#8B5CF6" icon="✅" delay={500} />
            </div>

            <div style={{ marginBottom: 8, fontSize: 11, color: MUTED, textTransform: "uppercase", letterSpacing: 1 }}>Saúde das Campanhas</div>
            <div style={{ background: CARD, border: `1px solid ${ORANGE}22`, borderRadius: 16, padding: 16, marginBottom: 16 }}>
              <HealthBar label="ROAS (meta: 4x)" value={`${metricas.roas}x`} max={10} color={metricas.roas >= 4 ? GREEN : YELLOW} />
              <HealthBar label="CTR (meta: 2%)" value={`${metricas.ctr}%`} max={5} color={metricas.ctr >= 2 ? GREEN : YELLOW} />
              <HealthBar label="Conversão (meta: 3%)" value={`${metricas.conversao}%`} max={5} color={metricas.conversao >= 3 ? GREEN : YELLOW} />
            </div>

            <div style={{ fontSize: 11, color: MUTED, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>🔍 Diagnóstico Automático</div>
            {diagnostico().map((d, i) => (
              <div key={i} style={{ background: `${d.cor}15`, border: `1px solid ${d.cor}44`, borderRadius: 12, padding: "10px 14px", marginBottom: 8, fontSize: 12.5, color: TEXT, lineHeight: 1.6 }}>{d.texto}</div>
            ))}

            <button onClick={() => setAba("editar")} style={{ width: "100%", background: SURFACE, border: `1px solid ${ORANGE}44`, borderRadius: 14, padding: "13px 0", color: ORANGE, fontSize: 13, fontWeight: 700, cursor: "pointer", marginTop: 8 }}>
              ✏️ Atualizar Métricas da Campanha
            </button>
          </div>
        )}
        ...
        {aba === "editar" && (
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, color: TEXT, marginBottom: 4 }}>✏️ Inserir Métricas</div>
            <div style={{ fontSize: 12, color: MUTED, marginBottom: 16 }}>Dados salvos no seu celular automaticamente</div>
            <MetricasForm metricas={metricas} onSave={salvarMetricas} />
          </div>
        )}

        {aba === "inicio" && (
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, color: TEXT, marginBottom: 4 }}>🚀 Como Iniciar um Anúncio</div>
            <div style={{ fontSize: 12, color: MUTED, marginBottom: 16 }}>Roteiro dos primeiros 14 dias de qualquer campanha nova</div>
            {tips.inicio.map((tip, i) => <TipCard key={i} tip={tip} index={i} />)}
            <div style={{ background: `linear-gradient(135deg, ${ORANGE}22, ${CARD})`, border: `1px solid ${ORANGE}44`, borderRadius: 16, padding: 16, marginTop: 8 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: ORANGE, marginBottom: 10 }}>📋 Checklist de Lançamento</div>
              {["Imagem principal com fundo branco e produto nítido", "Título com palavra-chave principal nos primeiros 30 chars", "Ao menos 5 fotos de produto + 1 vídeo curto", "Preço competitivo (top 3 da categoria)", "Estoque suficiente para rodar 30 dias", "Budget mínimo: R$10/dia para começar"].map((item, i) => (
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8, alignItems: "center" }}>
                  <div style={{ width: 18, height: 18, borderRadius: 4, background: `${ORANGE}33`, border: `1px solid ${ORANGE}`, flexShrink: 0 }} />
        ...
        {aba === "escala" && (
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, color: TEXT, marginBottom: 4 }}>📈 Estratégias de Escala</div>
            <div style={{ fontSize: 12, color: MUTED, marginBottom: 16 }}>Para campanhas que já funcionam e estão prontas para crescer</div>
            {tips.escala.map((tip, i) => <TipCard key={i} tip={tip} index={i} />)}
            <div style={{ background: CARD, border: `1px solid ${YELLOW}44`, borderRadius: 16, padding: 16, marginTop: 8 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: YELLOW, marginBottom: 12 }}>⚠️ Sinais de Alerta — Reduza o Budget</div>
              {[{ sinal: "ROAS cai abaixo de 2x", acao: "Pause e revise segmentação" }, { sinal: "CPC aumentou +50%", acao: "Reduza lance 20%" }, { sinal: "CTR caiu abaixo de 0,5%", acao: "Troque a imagem principal" }, { sinal: "Conversão < 1%", acao: "Revise preço e avaliações" }].map((item, i) => (
                <div key={i} style={{ marginBottom: 10, padding: 10, background: SURFACE, borderRadius: 10 }}>
                  <div style={{ fontSize: 12, color: RED, fontWeight: 600 }}>🔴 {item.sinal}</div>
                  <div style={{ fontSize: 11, color: MUTED, marginTop: 3 }}>→ {item.acao}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {aba === "ia" && (
          <div style={{ height: "calc(100vh - 200px)", display: "flex", flexDirection: "column" }}>
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 18, fontWeight: 800, color: TEXT, marginBottom: 4 }}>🤖 Consultor com IA</div>
              <div style={{ fontSize: 12, color: MUTED }}>Histórico salvo no celular • Usa suas métricas reais</div>
            </div>
            <div style={{ flex: 1, overflow: "hidden" }}>
              <AIPanel metricas={metricas} />
            </div>
          </div>
        )}
      </div>
      ...
      {/* BOTTOM NAV */}
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: CARD, borderTop: `1px solid ${ORANGE}22`, display: "flex", padding: "8px 0 14px", zIndex: 100 }}>
        {ABAS.map(a => (
          <button key={a.id} onClick={() => setAba(a.id)} style={{ flex: 1, background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "4px 0", position: "relative" }}>
            {aba === a.id && <div style={{ position: "absolute", top: -8, width: 32, height: 3, background: ORANGE, borderRadius: "0 0 4px 4px" }} />}
            <span style={{ fontSize: 18 }}>{a.icon}</span>
            <span style={{ fontSize: 9, color: aba === a.id ? ORANGE : MUTED, fontWeight: aba === a.id ? 700 : 400 }}>{a.label}</span>
          </button>
        ))}
      </div>
    </div>
